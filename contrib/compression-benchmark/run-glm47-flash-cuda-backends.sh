#!/usr/bin/env bash
# Compare CRIU CUDA backends with GLM-4.7-Flash on a single H200.
set -euo pipefail

usage() {
    cat <<'HELP'
Usage: sudo ./contrib/compression-benchmark/run-glm47-flash-cuda-backends.sh [RESULTS_DIR]
...
Uses the native BF16 weights with thinking disabled and a 32-token output limit.
...
if (( $# == 1 )); then
    ...
    results_dir=$(mktemp -d /var/tmp/glm47-flash-cuda-backends.XXXXXXXX)
fi
...
revision = os.environ.get("MODEL_REVISION", "7dd20894a642a0aa287e9827cb1a1f7f91386b67")
...
python3 "$script_dir/podman-sglang.py" \
    ...
    --sglang-arg=--reasoning-parser=glm45 \
    --offline \
    --warmup-requests 0 \
