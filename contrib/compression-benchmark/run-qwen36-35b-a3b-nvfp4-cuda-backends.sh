#!/usr/bin/env bash
# Compare CRIU CUDA backends with Qwen3.6-35B-A3B-NVFP4 on a single H200.
set -euo pipefail

usage() {
    cat <<'HELP'
Usage: sudo ./contrib/compression-benchmark/run-qwen36-35b-a3b-nvfp4-cuda-backends.sh [RESULTS_DIR]
...
Uses mixed FP8/NVFP4 weights with Marlin W4A16 kernels on H200.
...
if (( $# == 1 )); then
    ...
    results_dir=$(mktemp -d /var/tmp/qwen36-35b-a3b-nvfp4-cuda-backends.XXXXXXXX)
fi
...
revision = os.environ.get("MODEL_REVISION", "1355db6a052410cfd62085d94b58866fd0f2c3c5")
