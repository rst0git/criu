#!/usr/bin/env bash
# Compare CRIU CUDA backends with NVIDIA-Nemotron-3-Nano-4B-BF16 on a single H200.
set -euo pipefail

usage() {
    cat <<'HELP'
Usage: sudo ./contrib/compression-benchmark/run-nemotron3-nano-4b-bf16-cuda-backends.sh [RESULTS_DIR]
...
Uses native BF16 weights and FA3 attention on H200.
...
if (( $# == 1 )); then
    ...
    results_dir=$(mktemp -d /var/tmp/nemotron3-nano-4b-bf16-cuda-backends.XXXXXXXX)
fi
...
revision = os.environ.get("MODEL_REVISION", "dfaf35de3e30f1867dd8dbc38a7fc9fb52d3914f")
