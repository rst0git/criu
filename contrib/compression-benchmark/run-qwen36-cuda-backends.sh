#!/usr/bin/env bash
# Compare CRIU CUDA backends with Qwen3.6-27B on a single H200.
set -euo pipefail

usage() {
    cat <<'HELP'
Usage: sudo ./contrib/compression-benchmark/run-qwen36-cuda-backends.sh [RESULTS_DIR]
...
Optional environment:
  MODEL_REVISION  Full model commit hash to reuse; otherwise resolve it once
                 from Hugging Face and save it before starting the benchmark.
...
if (( $# == 1 )); then
    ...
    results_dir=$(mktemp -d /var/tmp/qwen36-cuda-backends.XXXXXXXX)
fi
...
if not revision:
    with urllib.request.urlopen(
        "https://huggingface.co/api/models/Qwen/Qwen3.6-27B", timeout=60
    ) as response:
