#!/usr/bin/env bash
# Compare CRIU CUDA backends with Qwen3.8-27B-FP8 on a single H200.
set -euo pipefail

usage() {
    cat <<'HELP'
Usage: sudo ./contrib/compression-benchmark/run-qwen38-fp8-cuda-backends.sh [RESULTS_DIR]
...
if (( $# == 1 )); then
    ...
    results_dir=$(mktemp -d /var/tmp/qwen38-fp8-cuda-backends.XXXXXXXX)
fi
...
revision = os.environ.get("MODEL_REVISION", "017b9c7af6b5689d5dd426a76e0bc077eb5ca20a")
