...
struct pid_info {
	int pid;
	char checkpointed;
	cuda_task_state_t original_state;
...
int cuda_plugin_checkpoint_devices(int pid)
{
	...
	list_for_each_entry(task_info, &cuda_pids, list) {
		if (task_info->pid == restore_tid) {
			if (task_info->original_state == CUDA_TASK_CHECKPOINTED) {
...
int cuda_plugin_pause_devices(int pid)
{
	...
	if (task_state == CUDA_TASK_LOCKED) {
		...
		add_pid_to_buf(&cuda_pids, restore_tid, CUDA_TASK_LOCKED);
		return 0;
	}
	...
	if (task_state == CUDA_TASK_CHECKPOINTED) {
		...
		add_pid_to_buf(&cuda_pids, restore_tid, CUDA_TASK_CHECKPOINTED);
		return 0;
	}
	...
	if (add_pid_to_buf(&cuda_pids, restore_tid, CUDA_TASK_RUNNING)) {
...
int resume_device(int pid, int checkpointed, cuda_task_state_t original_state)
{
	char msg_buf[CUDA_CKPT_BUF_SIZE];
	int status;
	int ret = 0;
	...
	if (original_state == CUDA_TASK_UNKNOWN) {
	...
	if (checkpointed && (original_state == CUDA_TASK_RUNNING || original_state == CUDA_TASK_LOCKED)) {
		/* If the process was locked or running before we checkpoint it, we need to restore it */
	...
	if (original_state == CUDA_TASK_RUNNING) {
...
int cuda_plugin_resume_devices_late(int pid)
{
	 ...
	 * to be in a running after restore, even if it was
...
void cuda_plugin_fini(int stage, int ret)
{
	...
	if (stage == CR_PLUGIN_STAGE__DUMP && (opts.final_state == TASK_ALIVE || ret != 0)) {
		...
		list_for_each_entry(info, &cuda_pids, list) {
			resume_device(info->pid, info->checkpointed, info->original_state);
		}
	}
	if (stage == CR_PLUGIN_STAGE__DUMP) {
