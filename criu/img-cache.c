...
static int unpack_entry(RemoteImageEntry *ri)
{
	...
	if (e->extra_data_size) {
		...
		if (ret != e->extra_data_size) {
			pr_perror("Recieved only %llu expected %lu", ret, e->extra_data_size);
			goto err;
		}
		pr_debug("Recieved extra data: %lu\n", e->extra_data_size);
	}
...
static int recieve_raw_image(RemoteImageEntry *ri)
{
	...
	if (ret != ri->size) {
		pr_perror("Recieved only %lu expected %llu", ret, ri->size);
		return -1;
	}
...
static int store_image(void)
{
	...
	if (!ri->has_fd_type || ri->fd_type > CR_FD_MAX) {
		pr_err("Invalid image fd_type=%u\n", ri->fd_type);
		goto out;
	}
	if (!ri->has_pb_type) {
