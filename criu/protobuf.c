...
int do_pb_read_one(struct cr_img *img, void **pobj, int type, bool eof)
{
	...
	*pobj = NULL;
	...
	if (opts.tls && type != PB_CIPHER && size > 0) {
		/* Read tag data */
		ret = bread(&img->_x, tag_data, sizeof(tag_data));
		if (ret < 0) {
			pr_perror("Can't read %d bytes of tag data from file %s", (int)sizeof(tag_data),
				  image_name(img, img_name_buf));
			goto err;
		} else if (ret != sizeof(tag_data)) {
			pr_perror("Read %d bytes of tag data while %d expected from %s", ret, (int)sizeof(tag_data),
				  image_name(img, img_name_buf));
			ret = -1;
			goto err;
		}
		...
		if (ret < 0) {
			pr_perror("Can't read %d bytes of nonce data from file %s", (int)sizeof(nonce_data),
				  image_name(img, img_name_buf));
			goto err;
		} else if (ret != sizeof(nonce_data)) {
			pr_perror("Read %d bytes of nonce data while %d expected from %s", ret, (int)sizeof(nonce_data),
				image_name(img, img_name_buf));
			ret = -1;
			goto err;
		}
