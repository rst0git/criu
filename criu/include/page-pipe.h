...
struct page_pipe_buf {
	...
	uint64_t pipe_off;	/* where this buf is started in a pipe */
	uint64_t pages_in;	/* how many pages are there */
#define PPB_LAZY (1 << 0)
	unsigned int flags;
...
extern int page_pipe_read(struct page_pipe *pp, struct pipe_read_dest *prd, unsigned long addr, uint64_t *nr_pages,
