...
int tls_initialize_cipher_from_image(void)
{
	...
	gnutls_datum_t default_chacha20_poly1305_key_ciphertext;
	gnutls_datum_t default_aes_key_ciphertext;
	...
	for (int i = 0; i < ce->tokens->n_key_entries; i++) {
		if (is_default_key_entry(ce->tokens->key_entries[i])) {
			default_chacha20_poly1305_key_ciphertext.size = ce->tokens->key_entries[i]->chacha20_poly1305_key.len;
			default_chacha20_poly1305_key_ciphertext.data = ce->tokens->key_entries[i]->chacha20_poly1305_key.data;
			default_aes_key_ciphertext.data = ce->tokens->key_entries[i]->aes_key.data;
			default_aes_key_ciphertext.size = ce->tokens->key_entries[i]->aes_key.len;
		...
		list_for_each_entry(key, &key_map_list, list) {
			...
			ciphertext.data = ce->tokens->key_entries[i]->aes_key.data;
			ciphertext.size = ce->tokens->key_entries[i]->aes_key.len;
	...
	if (!key_found) {
		...
		ret = gnutls_privkey_decrypt_data(privkey, 0, &default_chacha20_poly1305_key_ciphertext, &decrypted_token);
		if (ret < 0) {
			tls_perror("Failed to decrypt token data", ret);
			goto out_close;
		}
		...
		/* Decrypt AES key */
		ret = gnutls_privkey_decrypt_data(privkey, 0, &default_aes_key_ciphertext, &decrypted_token);
		if (ret < 0) {
			tls_perror("Failed to decrypt key data", ret);
			goto out_close;
		}
	...
	/* Decrypt AES key */
	ciphertext.data = ce->tokens->key_entries[0]->aes_key.data;
	ciphertext.size = ce->tokens->key_entries[0]->aes_key.len;
...
int write_img_cipher(void)
{
	...
	if (default_key_exists) {
		...
		ke->aes_key.len = ciphertext.size;
		ke->aes_key.data = (uint8_t*)xmalloc(ciphertext.size);
		if (!ke->aes_key.data) {
			// FIXME: free memory
			return -1;
		}
		memcpy(ke->aes_key.data, ciphertext.data, ciphertext.size);
	...
	cipher_entry.aes_iv.len = aes_xts_iv.size;
	cipher_entry.aes_iv.data = aes_xts_iv.data;

	/* Save HMAC digest */
	cipher_entry.hmac_digest.len = HMAC_SIZE;
	cipher_entry.hmac_digest.data = checkpoint_hmac_digest;
