...
static inline void pagemap_bound_check(PagemapEntry *pe, unsigned long vaddr, uint64_t nr)
{
	if (vaddr < pe->vaddr || (vaddr - pe->vaddr) / PAGE_SIZE + nr > pe->nr_pages) {
		pr_err("Page read err %" PRIx64 ":%" PRIu64 " vs %lx:%" PRIu64 "\n", pe->vaddr, pe->nr_pages, vaddr, nr);
		BUG();
	}
}

static int read_parent_page(struct page_read *pr, unsigned long vaddr, uint64_t nr, void *buf, unsigned flags)
{
	...
	do {
		...
		pr_info("\tparent has %" PRIu64 " pages in\n", p_nr);
		if (p_nr > nr)
			p_nr = nr;
...
static int maybe_read_page_local(struct page_read *pr, unsigned long vaddr, uint64_t nr, void *buf, unsigned flags)
{
	int ret;
 ...
 */
static int maybe_read_page_img_streamer(struct page_read *pr, unsigned long vaddr, uint64_t nr, void *buf, unsigned flags)
{
	unsigned long len = nr * PAGE_SIZE;
	int fd;
	int ret;
...
static int read_page_complete(unsigned long img_id, unsigned long vaddr, uint64_t nr_pages, void *priv)
{
	int ret = 0;
...
static int maybe_read_page_remote(struct page_read *pr, unsigned long vaddr, uint64_t nr, void *buf, unsigned flags)
{
	int ret;
...
static int read_pagemap_page(struct page_read *pr, unsigned long vaddr, uint64_t nr, void *buf, unsigned flags)
{
	pr_info("pr%lu-%u Read %lx %" PRIu64 " pages\n", pr->img_id, pr->id, vaddr, nr);
